...
func TestGraph_Extensions(t *testing.T) {
	...
	dataProto, ok := g.Lookup("extensions/ext/data.proto")
	require.True(t, ok)
	assert.NotNil(t, dataProto.(File).DefinedExtensions())
	assert.Len(t, dataProto.(File).DefinedExtensions(), 6)

	request, ok := g.Lookup(".extensions.Request")
	require.True(t, ok)
	assert.NotNil(t, request.(Message).DefinedExtensions())
	assert.Len(t, request.(Message).DefinedExtensions(), 1)
	assert.Empty(t, request.(Message).Extensions())

	msgOptions, ok := g.Lookup(".google.protobuf.MessageOptions")
	require.True(t, ok)
	assert.NotNil(t, msgOptions.(Message).Extensions())
	assert.Len(t, msgOptions.(Message).Extensions(), 1)

	everythingProto, ok := g.Lookup("extensions/everything.proto")
	require.True(t, ok)
	assert.Len(t, everythingProto.(File).Extensions(), 1)

	rootMsg, ok := g.Lookup(".extensions.RootMessage")
	require.True(t, ok)
	assert.Len(t, rootMsg.Extensions(), 1)
